...
OCT_subcomponentID hitBox2D_generateVertices(OCT_gameObjectID parentIndex) {
	iOCT_hitBox2D* hitBox = hitBox2D_get(parentIndex);
	OCT_vector2D hitBoxWidth = { hitBox->size.x, 0 };
	OCT_vector2D hitBoxHeight = { 0, hitBox->size.y };
	OCT_vector2D totalOffset = OCT_vector2D_Vector2D(OCT_OP_ADD, hitBox->localOrigin, position2D_get(parentIndex)->globalPosition2D);
