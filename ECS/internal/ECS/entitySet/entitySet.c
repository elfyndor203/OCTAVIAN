...
iOCT_entityContext iOCT_entitySetPool_default[iOCT_ENTITYSET_DEFAULT_MAX];
OCT_counter iOCT_entityContextCounter = 0;
static const iOCT_ECS iOCT_GAME_ECS = { 
	iOCT_entitySetMap_default, 
	iOCT_entitySetPool_default, 
	&iOCT_entityContextCounter };
...
iOCT_entityContext* iOCT_entitySet_get(iOCT_entityContextID entitySetID) {				// valid as long as the entitySet exists
	if (entitySetID >= iOCT_entityContextCounter) {
...
/// </summary>
static iOCT_pool iOCT_pool_new(iOCT_entityContext* entitySet, OCT_componentTypes componentType) {
	iOCT_pool newPool = { 0 };											// create and set values

	newPool.entitySetID = entitySet->entitySetID;
	newPool.componentType = componentType;
	newPool.counter = 0;
	newPool.capacity = iOCT_POOLSIZE_DEFAULT;
...
static iOCT_IDMap iOCT_IDMap_new(iOCT_entityContext* entitySet) {
	iOCT_IDMap newMap = { 0 };
...
static iOCT_entityID iOCT_rootEntity_new(iOCT_entityContext* entitySet) {
	...
	iOCT_entityContextID entitySetID = entitySet->entitySetID;
...
OCT_entityHandle iOCT_entityContext_open() {
	...
	newEntityContext.entitySetID = entitySetID;					// set first to ensure the rootEntity gets the correct info

	for (int component = 0; component < OCT_componentsTotal; component++) {			// create and log each pool type
		newEntityContext.pools[component] = iOCT_pool_new(&newEntityContext, component);
	}
	newEntityContext.IDmap = iOCT_IDMap_new(&newEntityContext);
...
iOCT_pool* iOCT_pool_get(iOCT_entityContextID entitySetID, OCT_componentTypes componentType) {	// If the set exists, get the poolID from it. If the pool exists, return it. 
