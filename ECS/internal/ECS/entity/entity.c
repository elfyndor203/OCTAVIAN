...
OCT_entityHandle OCT_entity_new(OCT_entityHandle parentHandle) {
	OCT_ID contextID = parentHandle.entityContextID;
	...
	OCT_entityHandle newHandle = { contextID, newEntityID };
	return newHandle;
}
OCT_ID iOCT_entity_new(iOCT_entityContext* context, OCT_ID parentID) {
	...
	newID = iOCT_IDMap_registerID(context, OCT_typeEntity);		// Register an ID first to enable other functions
	...
	newEntity = iOCT_getByID(context, newID, OCT_typeEntity);	// Get the empty pool slot
	memset(newEntity, 0, sizeof(iOCT_entity));							// Make sure the slot is empty

	newEntity->entityContextID = context->entityContextID;
...
bool OCT_entity_hasComponent(OCT_entityHandle entityHandle, OCT_types component) {
	...
	return iOCT_entity_hasComponent(context, entityHandle.entityID, component);
}
