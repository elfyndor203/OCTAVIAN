...
void testKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_RIGHT:
			...
			_OCT_sendMessage(OCT_subsystem_ECS, _OCT_active, _OCT_position2D_move, .1f, 0);
			break;
		case GLFW_KEY_LEFT:
		...
		case GLFW_KEY_UP:
			...
			_OCT_sendMessage(OCT_subsystem_ECS, _OCT_active, _OCT_position2D_move, 0.0f, 0.1f);
			break;
		case GLFW_KEY_DOWN:
		...
		case GLFW_KEY_RIGHT:
			...
			_OCT_sendMessage(OCT_subsystem_ECS, _OCT_active, _OCT_position2D_stop, 0, 0);
		}
	}
}