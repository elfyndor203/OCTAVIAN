...
OCT_rendererObjectID OCT_rendererObject_new(size_t engineIndex, GLRequest VAORequest, GLRequest VBORequest, GLRequest EBORequest, bool is3D, bool dynamic) {	// determines whether or not to generate a new VAO
...
OCT_rendererObjectID rendererObject_new(size_t engineIndex, float* vertexArray, size_t vertexCount, uint* indexArray, size_t indexCount, bool is3D, bool isDynamic) {	// makes it so macros are not needed externally
	OCT_rendererObjectID newRendererObject;
	if (indexArray != OCT_ID_EBO_NONE) {
		newRendererObject = OCT_rendererObject_new(engineIndex, VAO_NEW, VBO_NEW(vertexArray, vertexCount), EBO_NEW(indexArray, indexCount), is3D, isDynamic);
	}
	else {
		newRendererObject = OCT_rendererObject_new(engineIndex, VAO_NEW, VBO_NEW(vertexArray, vertexCount), EBO_NONE, is3D, isDynamic);
	}
	return newRendererObject;
}

void rendererObject_update(OCT_rendererObjectID object) {
...
void OCT_TEST_RENDER_OBJECT(OCT_rendererObjectID objectToRender) {
	glBindVertexArray(rendererObject_get(objectToRender)->VAO);
	uint vertex = vertexShader_initialize(hitBox2D_vertexShaderSource);
	uint fragment = fragmentShader_initialize(hitBox2D_fragmentShaderSource);
	uint testProgram = shaderProgram_create(vertex, fragment);
